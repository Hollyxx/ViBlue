...
public class TopUpRechargeActivity extends BaseActivity implements MyHttpUtils.MyHttpCallback,View.OnClickListener{
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        myDialog.showNoticeDialog(TopUpRechargeActivity.this, "恭喜你，押金充值成功！", TopUpRechargeActivity.this,false);
                        myDialog.show();
                    } else {
                        // 该笔订单真实的支付结果，需要依赖服务端的异步通知。
                        Toast.makeText(TopUpRechargeActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
                        hud.dismiss();
                    }
                    break;
                }
            ...
            case Connect.DEPOSIT:
                if (getCode(result) == 0) {
                    try {
                        ...
                        //判断是微信支付  还是支付宝支付
                        if (pay_way == 1) {  //支付宝
                            Connect.aliPay(TopUpRechargeActivity.this, data.getString("pdr_sn"), this);
                        } else {//微信
            ...
            case Connect.WX_PAY_CHARGE_DEPOSIT://微信
                if (getCode(result) == 0) {
                    if (!isWXAppInstalledAndSupported()) {
                        Toast.makeText(TopUpRechargeActivity.this, "请安装最新版本的微信端", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    try {
                ...
                if (getCode(result) == 0) {
                    try {
                        ...
                        Runnable payRunnable = new Runnable() {
                            ...
                            @Override
                            public void run() {
                                PayTask alipay = new PayTask(TopUpRechargeActivity.this);
    ...
    public void onError(String errorMsg, int whereRequest) {
        ...
        switch (whereRequest) {
            case Connect.DEPOSIT:
                ToastUtils.showShort(TopUpRechargeActivity.this, "获取订单号失败");
                break;
            case Connect.WX_PAY_CHARGE_DEPOSIT:
                ToastUtils.showShort(TopUpRechargeActivity.this, errorMsg);
                break;
    ...
    public void close(String event) {
        if ("recharge_deposit".equals(event)) {
            myDialog = new MyDialog();
            myDialog.showNoticeDialog(TopUpRechargeActivity.this, "恭喜你，押金充值成功！", TopUpRechargeActivity.this,false);
            myDialog.show();
